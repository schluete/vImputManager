...
^H  (or <BS>) Same as left arrow. (See <h>). During an insert, eliminates the last input character, backing 
    over it but not erasing it; it remains so the user can see what he typed if he wishes to type something only 
...
^J  (or <LF>) Same as down arrow (see <j>). 

^L  The ASCII formfeed character, this causes the screen to be cleared and redrawn. This is useful after a 
    transmission error, if characters typed by a program other than the editor scramble the screen, or after 
    output is stopped by an interrupt (5.4,7.2f). 
...
/   Reads a string from the last line on the screen, and scans forward for the next occurrence of this 
    ...
    end-around to nd a string anywhere in the buffer. 
...
c   An operator which changes the following object, replacing it with the following input text up to an <ESC>. If
    ...
    with a$. A count causes that many objects to be affected, thus both 3c)  and c3) change the following three 
    sentences (7.4). 
...
h   (or <Left arrow>) Moves the cursor one character to the left. Like the other arrow keys, either h, the left 
    arrow key, or one of the synonyms (^H) has the same effect. A count repeats the effect (3.1,7.5). 

i   Inserts text before the cursor, otherwise like a (7.2). 

j   (or <Down arrow>) Moves the cursor one line down in the same column. If the position does not exist, vi 
    comes as close as possible to the same column. Synonyms include ^J (linefeed) and ^N. 

k   (or <Uparrow>) Moves the cursor one line up. ^P is a synonym.

l   (or <Right arrow>) Moves the cursor one character to the right. <SPACE> is a synonym. 
...
y   An operator, yanks the following object into the unnamed temporary buffer. If preceded by a named buffer 
    specification, "x, the text is placed in that buffer also. Text can be recovered by a later <p> or <P> (7.4). 
